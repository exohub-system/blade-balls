...
local function findBall()
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") and (obj.Name == "Ball" or obj.Name == "BladesBall" or obj.Name == "KillBrick") then
            return obj
        end
    end
end
    ...
    parryConn = RunService.Heartbeat:Connect(function()
        ...
        if dist < 15 then
            pcall(function()
                ...
                if remotes then
                    local parryRemote = remotes:FindFirstChild("Parry") or remotes:FindFirstChild("Block") or remotes:FindFirstChild("Deflect")
                    if parryRemote then
                        parryRemote:FireServer()
                    end
                end
                local parryBtn = player.PlayerGui:FindFirstChild("Parry", true) or player.PlayerGui:FindFirstChild("Block", true)
                if parryBtn and parryBtn:IsA("TextButton") then
                    parryBtn.MouseButton1Click:Fire()
                end
            end)
        end
    end)
...
local function setSpeed(on)
    ...
    if hum then
        hum.WalkSpeed = on and 60 or speedDefault
    end
end
    ...
    skillConn = RunService.Heartbeat:Connect(function()
        ...
        if dist < 30 then
            pcall(function()
                local remotes = ReplicatedStorage:FindFirstChild("Remotes") or ReplicatedStorage:FindFirstChild("Events")
                if remotes then
                    local skillRemote = remotes:FindFirstChild("Skill") or remotes:FindFirstChild("Ability") or remotes:FindFirstChild("UseSkill")
                    if skillRemote then skillRemote:FireServer() end
                end
            end)
        end
    end)
...
slash3.BackgroundColor3 = Color3.fromRGB(255, 100, 130)
...
discordLabel.Position = UDim2.new(0, 0, 0, 52)
...
local features = {
    {
        key = "autoParry",
        name = "Auto Parry",
        desc = "Auto deflects ball at perfect frame",
        icon = "🛡️",
    ...
    },
    {
        key = "speed",
        name = "Speed Hack",
        desc = "Move insanely fast to dodge",
        icon = "⚡",
    ...
    },
    {
        key = "fly",
        name = "Fly",
        desc = "Fly above the ball — WASD to move",
        icon = "🛸",
    ...
    },
    {
        key = "ballESP",
        name = "Ball ESP",
        desc = "Always see where the ball is",
        icon = "👁",
    ...
    },
    {
        key = "autoSkill",
        name = "Auto Skill",
        desc = "Auto uses ability when ball is close",
        icon = "✨",
    ...
    },
    {
        key = "antiDie",
        name = "Anti Die",
        desc = "Cannot be killed by the ball",
        icon = "💀",
        color = Color3.fromRGB(255, 140, 0),
        onEnable = function() setupAntiDie(player.Character) end,
...
local function makeCard(feature, index)
    ...
    toggleBtn.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(pill, TweenInfo.new(0.2), {
            BackgroundColor3 = on and feature.color or Color3.fromRGB(20, 10, 35)
        }):Play()
    ...
    card.MouseEnter:Connect(function()
        TweenService:Create(card, TweenInfo.new(0.15), {BackgroundTransparency = 0.85}):Play()
    end)
    card.MouseLeave:Connect(function()
        TweenService:Create(card, TweenInfo.new(0.15), {BackgroundTransparency = 0.92}):Play()
    end)
end
...
discordStroke.Parent = discordBanner
...
discordText.Size = UDim2.new(1, -50, 0, 18)
discordText.Position = UDim2.new(0, 42, 0, 4)
...
discordLink.Size = UDim2.new(1, -50, 0, 14)
discordLink.Position = UDim2.new(0, 42, 0, 24)
discordLink.BackgroundTransparency = 1
discordLink.Text = "discord.gg/TzNds43vb"
discordLink.TextColor3 = Color3.fromRGB(180, 190, 255)
discordLink.TextSize = 10
discordLink.Font = Enum.Font.Gotham
discordLink.TextXAlignment = Enum.TextXAlignment.Left
discordLink.Parent = discordBanner
